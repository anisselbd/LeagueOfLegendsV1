...
export async function getLatestVersion() { // Récupère la dernière version disponible de League of Legends
  const { data } = await axios.get(DDRAGON + "/api/versions.json");
  return data[0]; 
}

export async function getChampionList(version, locale = "fr_FR") { // Récupère la liste des champions pour une version et une locale données
  const url = DDRAGON + "/cdn/" + version + "/data/" + locale + "/champion.json";
  const { data } = await axios.get(url);
  return Object.values(data.data); 
}

export function championSquare(version, imageName) { // Génère l'URL de l'icône carrée d'un champion
...
export function splashUrl(champId, skinNum = 0) { // Génère l'URL de l'image splash d'un champion pour un skin donné
  return DDRAGON + "/cdn/img/champion/splash/" + champId + "_" + skinNum + ".jpg";
}
export function loadingUrl(champId, skinNum = 0) { // Génère l'URL de l'image de chargement d'un champion pour un skin donné
  return DDRAGON + "/cdn/img/champion/loading/" + champId + "_" + skinNum + ".jpg";
}

export function spellIcon(version, fileName) { // Génère l'URL de l'icône d'un sort
